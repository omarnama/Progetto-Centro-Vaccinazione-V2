...
public class PersonaDao {
	 ...
	 * 
	 * boolean result = false; try (Session session =
	 * HibernateUtil.getSessionFactory().openSession()) {
	 * 
	 * if (session != null) { result = true; } } log.debug("result " + result);
	 * 
	 * return result; }
	 */
	 ...
	 */
	/*
	 * public Persona creaPersona(Persona persona) throws ParseException{ Scanner n
	 * = new Scanner(System.in); Transaction tx = null; Session session =
	 * HibernateUtil.getSessionFactory().openSession();
	 * 
	 * 
	 * 
	 * try {
	 * 
	 * tx = session.beginTransaction(); //Persona persona = new Persona()
	 * 
	 * Query<Persona> query = session .createQuery("INSERT INTO Persona",
	 * Persona.class);//id, nome, cognome, data_nascita, recapito_telefonico,
	 * indirizzo_residenza, creation_time, creation_user
	 * System.out.println("Inserisci id persona:"); query.setParameter(0,
	 * persona.getId()); //query.setParameter(position, value)
	 * System.out.println("Inserisci nome persona:"); query.setParameter(1,
	 * persona.getNome()); System.out.println("Inserisci cognome persona:");
	 ...
	 * query.setParameter(5, persona.getIndirizzo_residenza()); System.out.
	 * println("Inserimento automatico dell'ora alla creazione dell record");
	 * query.setParameter(6, new
	 * java.sql.Timestamp(persona.getCreation_time().getTime()));//.getTime()
	 * System.out.
	 * println("Inserimento automatico dell'utente alla creazione dell record");
	 ...
	 * 
	 * 
	 * tx.commit(); return (Persona) query.list(); } catch (HibernateException e) {
	 * if (tx!=null) tx.rollback(); e.printStackTrace(); } finally {
	 * session.close(); } return persona;
	 * 
	 * 
	 * 
	 * 
	...
	// RENDERE PERSISTENTI LE MODIFICHE E AVVIARE MODIFICA IN DB, (con tx commit
	// forse)

	/*
	 * Dao update che viene richiamato da Service nel momento dell'update in db, di
	 * campi
	 */
	 ...
	 * try (Session session = HibernateUtil.getSessionFactory().openSession()) {
	 * 
	 * session.beginTransaction(); session.update(persona); }
	 * 
	 * }
	 */
